...
4. 分析并制定计划（新增/修改哪些词条，分类规则参见下文）
   → **必须同时识别规则**：境界/力量体系/组织架构/时间线等系统性设定 → 用 new_rule 提取（见「规则提取（重要）」）
...
**LLM 常见失误：只建人物/势力/物品词条，遗漏世界观规则。** 规则与 wiki 词条同等重要，不可跳过。

### 每章必查的规则信号

阅读每一章时，主动识别以下**系统性设定**，出现即用 `new_rule` 提取：

| 规则信号 | 示例 |
|---------|------|
| 境界/等级/修炼体系 | 肉仙十重、筑基→金丹→元婴 |
| 力量体系 | 魔法/斗气/灵力体系的构成与分支 |
| 势力组织结构 | 宗门的堂口/职位/晋升体系 |
| 时间线/纪年 | 朝代更迭、历史大事件顺序 |
| 世界底层设定 | 天道规则、灵气浓度、规则限制 |
| 品阶/货币/资源体系 | 丹药品阶、灵石等级、物价 |
| 禁忌/限制规则 | 天机不可泄露、越级限制 |

### 执行要求

1. **出现即提取**：章节中出现系统性设定描述（分级、分类、构成、机制），必须用 `new_rule` 提取为规则文档，不能只记录具体实体。
2. **多章聚合**：同一规则分散在多章描述时，汇总合并为一个规则文档；新章节出现相关描述时用 `edit_rule` 补充。
3. **判断口诀**：
   - 「这是世界的运转规律，不是故事里的具体角色/物品」→ 规则（new_rule）
   - 「这是故事中具体出现的人、物、地、事」→ wiki 词条（new_wiki）
4. **提交计划时必须包含规则**：submit_plan 的 plan_json 中，如果本章有规则信号但 `new_rule` 为空，请自查是否遗漏。
5. **审核时补查**：若发现某体系（境界/等级/组织架构）在正文反复出现但 rules/ 无对应文档，属于规则遗漏，必须在审核阶段补建。

> 规则提取优先级与 wiki 词条相同，禁止因"麻烦"而把规则性内容混入 wiki 词条或直接忽略。
...
**JSON 结构硬性规范（违反会导致词条静默丢失）**：

- 每个字段（`new_wiki` / `new_rule` / `new_plot` / `edit_wiki` 等）在 JSON 中**只能出现一次**，所有同类条目合并进同一个数组（用 `category` 区分类别）；**禁止拆成多个同名键**——JSON 重复键后值覆盖前值，系统只收到最后一个数组，其余词条全部丢失（用户只会看到最后几条）
- `new_wiki` 数量没有上限：单批可提交 30-60 条，覆盖范围内所有新出现的重要实体（人物/势力/地点/物品/功法/组织等），不要只建几条
- `new_plot` 每批控制在 **4 张以内**：只收录主线关键事件（开篇冲突/转折/高潮/结局），避免堆砌次要事件
- `new_rule` 只建真正的体系性规则（境界/力量体系/世界底层设定/禁忌等）；地点、势力、组织一律作为 wiki 词条，**不要建规则文档**
...
```json
{
  ...
  "new_wiki": [
    ...
    {"category": "势力", "name": "叶家", "chapters": "1", "reason": "主角家族"},
    {"category": "地图", "name": "紫玉大陆", "chapters": "1", "reason": "故事主舞台"}
  ],
  "edit_wiki": [
...
- 禁止：把 wiki 能解答的问题（如"叶匀什么修为"）变成章节阅读
...
| 规则文档检查 | 检查 rules/ 下的规则文档是否包含 `[[wikilink]]`，以及**是否遗漏重要规则**（境界/力量体系/组织架构/时间线等系统性设定在正文反复出现时，必须有对应规则文档） |
...
- **输出规范：与用户沟通和输出任何文本时禁止使用 emoji 表情符号**，使用纯文本或方括号标记（如 [警告]、[重要]）
